...
public class XPathBaseHelper {
  ...
  public static String go(String filePath, String xpath) {
    ...
    try {
      XdmNode node = builder.build(new File(filePath));
      XPathCompiler compiler = processor.newXPathCompiler();
      getNamespaces().forEach(compiler::declareNamespace);
      XdmValue evaluated = compiler.evaluate(xpath, node);
  ...
  private static Map<String, String> getNamespaces() {
    return new HashMap<String, String>() {{
      ...
      put("int", "urn:EEC:Interaction:v1.0");
      put("sgn", "urn:EEC:signal:v1.0");
